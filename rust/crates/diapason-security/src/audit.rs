...
impl AuditLogger {
    ...
    pub fn log(&self, event: &SecurityEvent) -> Result<(), DiapasonError> {
    ...
    pub fn verify_chain(&self) -> Result<(bool, Option<i64>), DiapasonError> {
        let mut stmt = self
            .conn
            .prepare(
        ...
        for row_result in rows {
            let (rid, ts, etype, fj, preview, action, stored_hash, stored_prev) =
                row_result.map_err(|e| DiapasonError::Io(std::io::Error::other(e.to_string())))?;
    ...
    pub fn query(
        ...
        since: Option<f64>,
        limit: usize,
    ) -> Result<Vec<SecurityEvent>, DiapasonError> {
