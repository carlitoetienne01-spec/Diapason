...
use crate::RUNTIME;
use diapason_agents::OjAgent;
use diapason_engine::rig_adapter::RigModelAdapter;
use diapason_engine::Engine;
use pyo3::prelude::*;
use std::sync::Arc;
...
enum AgentEnum {
    Simple(diapason_agents::SimpleAgent<DefaultAdapter>),
    Orchestrator(diapason_agents::OrchestratorAgent<DefaultAdapter>),
    NativeReAct(diapason_agents::NativeReActAgent<DefaultAdapter>),
}
...
impl PyMonitorOperativeAgent {
    ...
    fn new(
        ...
        let mem_ext = match memory_extraction {
            "scratchpad" => diapason_agents::MemoryExtraction::Scratchpad,
            "structured_json" => diapason_agents::MemoryExtraction::StructuredJson,
            "none" => diapason_agents::MemoryExtraction::None,
            _ => diapason_agents::MemoryExtraction::CausalityGraph,
        };
        let obs_comp = match observation_compression {
            "truncate" => diapason_agents::ObservationCompression::Truncate,
            "none" => diapason_agents::ObservationCompression::None,
            _ => diapason_agents::ObservationCompression::Summarize,
        };
        let ret_strat = match retrieval_strategy {
            "keyword" => diapason_agents::RetrievalStrategy::Keyword,
            "semantic" => diapason_agents::RetrievalStrategy::Semantic,
            "none" => diapason_agents::RetrievalStrategy::None,
            _ => diapason_agents::RetrievalStrategy::HybridWithSelfEval,
        };
        let task_dec = match task_decomposition {
            "monolithic" => diapason_agents::TaskDecomposition::Monolithic,
            "hierarchical" => diapason_agents::TaskDecomposition::Hierarchical,
            _ => diapason_agents::TaskDecomposition::Phased,
        };

        let monitor_config = diapason_agents::MonitorConfig {
        ...
        let agent = diapason_agents::MonitorOperativeAgent::new(
            adapter,
            executor,
            max_turns,
            temperature,
...
pub struct PyLoopGuard {
    inner: diapason_agents::LoopGuard,
}
...
impl PyLoopGuard {
    ...
    fn new(max_identical: usize, max_ping_pong: usize, poll_budget: usize) -> Self {
        Self {
            inner: diapason_agents::LoopGuard::new(max_identical, max_ping_pong, poll_budget),
        }
    }
