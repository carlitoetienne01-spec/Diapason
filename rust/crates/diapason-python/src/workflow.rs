...
pub struct PyWorkflowGraph {
    inner: diapason_workflow::WorkflowGraph,
}
...
impl PyWorkflowGraph {
    ...
    fn new(name: &str) -> Self {
        Self {
            inner: diapason_workflow::WorkflowGraph::new(name),
        }
    }
    ...
    fn add_node(
        ...
        let nt = match node_type {
            "tool" => diapason_workflow::NodeType::Tool,
            "condition" => diapason_workflow::NodeType::Condition,
            "parallel" => diapason_workflow::NodeType::Parallel,
            "loop" => diapason_workflow::NodeType::Loop,
            "transform" => diapason_workflow::NodeType::Transform,
            _ => diapason_workflow::NodeType::Agent,
        };
        let node = diapason_workflow::WorkflowNode {
            id: id.to_string(),
            node_type: nt,
    ...
    fn add_edge(&mut self, source: &str, target: &str) -> PyResult<()> {
        let edge = diapason_workflow::WorkflowEdge {
...
pub struct PyWorkflowBuilder {
    inner: Option<diapason_workflow::WorkflowBuilder>,
}
...
impl PyWorkflowBuilder {
    ...
    fn new(name: &str) -> Self {
        Self {
            inner: Some(diapason_workflow::WorkflowBuilder::new(name)),
        }
    }
