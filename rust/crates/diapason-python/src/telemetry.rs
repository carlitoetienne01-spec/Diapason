...
pub struct PyTelemetryStore {
    pub inner: Arc<diapason_telemetry::TelemetryStore>,
}
...
pub struct PyTelemetryAggregator {
    store: Arc<diapason_telemetry::TelemetryStore>,
}
...
impl PyTelemetryAggregator {
    ...
    fn stats(&self) -> PyResult<String> {
        let stats = diapason_telemetry::TelemetryAggregator::stats(&self.store)
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string()))?;
...
pub struct PyInstrumentedEngine {
    inner: diapason_telemetry::InstrumentedEngine<diapason_engine::Engine>,
}
...
pub struct PyTelemetrySessionCore {
    inner: diapason_telemetry::session::TelemetrySessionCore,
}
...
impl PyTelemetrySessionCore {
    ...
    fn new(capacity: usize, sampling_interval_ms: u64) -> Self {
        Self {
            inner: diapason_telemetry::session::TelemetrySessionCore::new(
                capacity,
    ...
    fn add_sample(&self, sample: &PyTelemetrySample) {
        let s = diapason_telemetry::session::TelemetrySample {
...
impl PyFlopsEstimator {
    ...
    fn estimate_flops(model: &str, input_tokens: u64, output_tokens: u64) -> (f64, f64) {
        diapason_telemetry::flops::estimate_flops(model, input_tokens, output_tokens)
    }

    #[staticmethod]
    #[pyo3(signature = (flops, duration_s, gpu_name, num_gpus=1))]
    fn compute_mfu(flops: f64, duration_s: f64, gpu_name: &str, num_gpus: u32) -> f64 {
        diapason_telemetry::flops::compute_mfu(flops, duration_s, gpu_name, num_gpus)
    }
}
...
impl PyPhaseMetrics {
    ...
    fn compute(
        ...
        let rust_samples: Vec<diapason_telemetry::session::TelemetrySample> = samples
            .iter()
            .map(|s| diapason_telemetry::session::TelemetrySample {
                timestamp_ns: s.timestamp_ns,
                gpu_power_w: s.gpu_power_w,
                cpu_power_w: s.cpu_power_w,
                gpu_energy_j: s.gpu_energy_j,
    ...
    fn split_at_ttft(
        ...
        let rust_samples: Vec<diapason_telemetry::session::TelemetrySample> = samples
            ...
            .collect();
        let (prefill, decode) = diapason_telemetry::phase::split_at_ttft(
            &rust_samples,
            start_ns,
            ttft_ns,
            end_ns,
