...
use diapason_core::{DiapasonError, Trace};
...
impl TraceStore {
    pub fn new(db_path: &Path) -> Result<Self, DiapasonError> {
        if let Some(parent) = db_path.parent() {
            std::fs::create_dir_all(parent)
                .map_err(|e| DiapasonError::Io(std::io::Error::other(e)))?;
        }
    ...
    pub fn save(&self, trace: &Trace) -> Result<(), DiapasonError> {
    ...
    pub fn get(&self, trace_id: &str) -> Result<Option<Trace>, DiapasonError> {
        let conn = self.conn.lock();
        let mut stmt = conn
            .prepare(
    ...
    pub fn list_traces(&self, limit: usize, offset: usize) -> Result<Vec<Trace>, DiapasonError> {
        let conn = self.conn.lock();
        let mut stmt = conn
            .prepare(
                "SELECT trace_id, query, agent, model, engine, result, outcome, feedback,
    ...
    pub fn count(&self) -> Result<usize, DiapasonError> {
        let conn = self.conn.lock();
        let count: i64 = conn
