...
This tutorial demonstrates how to use Diapason to run **autonomous scheduled agents** that perform recurring personal tasks on a cron-like schedule.

## What This Demonstrates

- Using the `Diapason` SDK with different agent types (`orchestrator`, `native_react`) and tool sets
- Configuring recurring schedules via TOML and the `diapason scheduler` CLI
...
Register each script as a recurring task with `diapason scheduler create`:
...
uv run python examples/scheduled_ops/daily_digest.py | diapason channel send slack
...
See `diapason channel list` for all available channels.
...
- **Model selection**: Use `--model` to target a specific model, or let Diapason auto-select from what is available.
