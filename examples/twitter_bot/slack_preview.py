...
TOPICS = [
    # Narrative — opinionated takes that happen to be about Diapason
...
def main() -> None:
    ...
    j = Diapason(model="qwen3:32b", engine_key="ollama")
    j._config.agent.max_turns = 5

    # Clean and index
    db_path = os.path.expanduser("~/.diapason/memory.db")
    ...
    parent_ts = slack_send(
        "*Diapason Twitter Bot — Overnight Preview*\n"
    ...
    while True:
        ...
        tool_log.clear()
        r = j.ask_full(
            "You are @DiapasonAI — a researcher/dev who believes local-first "
