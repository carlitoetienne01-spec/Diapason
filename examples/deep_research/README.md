...
using Diapason. The assistant uses an orchestrator agent loop with web
...
- Diapason installed (`uv sync --extra dev` from the repo root)
...
recipe that can be loaded with `load_recipe()` or passed to the `diapason eval`
runner.
...
User query
  |
  v
Diapason SDK  (model + engine selection)
  |
  v
...
- **Add more tools** -- append tool names to the `tools` list in
  `research.toml` or pass them on the command line. See `diapason agent info
...
- **Use the recipe programmatically** -- load the TOML with
  `diapason.recipes.load_recipe("examples/deep_research/research.toml")` and
...
- [Recipes](../../src/diapason/recipes/) -- composable TOML configs that
