...
A set of developer-focused scripts that use Diapason tool-using agents
to automate common coding tasks: code review, debugging, and test generation.

## What This Demonstrates

Each script wires up a `Diapason` instance with the `native_react` (ReAct) agent
...
1. **Install Diapason** (from the repo root):
...
```bash
python examples/code_companion/test_gen.py --module src/diapason/tools/calculator.py
```
...
`memory_store`, `memory_search`, and more. Run `uv run diapason eval list` or
inspect `src/diapason/tools/` for the full registry.
