...
Index a directory of documents into Diapason memory and answer questions
...
1. **Index** -- Uses `Diapason.memory.index()` to chunk the documents at
...
Diapason SDK. Adjust `--chunk-size` and `--top-k` to tune the
