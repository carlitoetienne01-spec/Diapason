...
Diapason orchestrator agent.

## What This Demonstrates

- **Channel integration** — connecting Diapason to messaging platforms (Slack, WhatsApp, etc.)
...
- Python 3.10+
- Diapason installed: `uv sync --extra dev`
...
2. Configure the WhatsApp Baileys bridge (see the Diapason channel docs).
...
Diapason supports many channels — LINE, Viber, Mastodon, Rocket.Chat, and
...
Refer to `configs/diapason/config.toml` for the full list of channel and agent
options.
...
this via the Diapason scheduler:

```bash
diapason scheduler create "Daily inbox summary" --type cron --value "0 17 * * *"
```

Or use the operator recipe pattern to run a persistent triage agent on a
schedule. See `src/diapason/recipes/data/operators/` for examples.
