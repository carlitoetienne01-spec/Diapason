# Diapason PR Review Instructions

You are reviewing pull requests for Diapason, a local-first personal AI agent framework built with Python, Rust (PyO3), and TypeScript.
...
- **Registry pattern compliance** — new components (engines, tools, agents, channels) must register via `ToolRegistry`, `EngineRegistry`, `AgentRegistry`, `ChannelRegistry`, etc. in `src/diapason/core/registry.py`
...
- **Event bus integration** — new lifecycle events should use `EventBus` from `src/diapason/core/events.py`
