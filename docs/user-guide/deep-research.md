...
This writes a pre-configured `~/.diapason/config.toml` for the deep research agent.
...
Diapason chunks the content and stores it in a local SQLite/FTS5 database. Supported formats include `.txt`, `.md`, `.pdf`, `.py`, `.json`, `.csv`, and more.
...
diapason ask "What meetings did I have with Alice last month?"

# Explicitly specify the agent
diapason ask --agent deep_research "Compare the approaches described in paper-a.pdf and paper-b.pdf"
...
diapason memory search "project timeline"
diapason memory search -k 20 "budget estimates"
...
diapason ask "Summarize all emails about the Q3 budget review"

# Cross-reference sources
diapason ask "What do papers A and B agree on regarding attention mechanisms?"

# Find specific information
diapason ask "What meetings did I have with Alice last month?"

# Extract structured data
diapason ask "List all action items from the meeting notes in ~/Documents/meetings/"

# Research with web fallback
diapason ask "Compare our internal benchmarks with the latest published results"
```
...
**"No results found"** -- Make sure you have indexed documents first with `diapason memory index`. Check indexed content with `diapason memory stats`.
