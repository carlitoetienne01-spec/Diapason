...
!!! note "Optional component"
    The scheduler is a standalone module (`diapason.scheduler`). It is not wired into the default `Diapason` / `DiapasonSystem` startup. You enable it explicitly via `SystemBuilder` or by starting the CLI daemon with `diapason scheduler start`.
...
The `diapason scheduler` subcommand group manages tasks and the daemon from the terminal.
...
# Wire in a DiapasonSystem for task execution
...
4. Starts a daemon thread named `"diapason-scheduler"`. Daemon threads exit automatically when the main process exits.
...
Scheduler settings live in the `[scheduler]` section of `~/.diapason/config.toml`.
...
| `db_path`        | `str`  | `~/.diapason/scheduler.db`     | SQLite database path                       |
