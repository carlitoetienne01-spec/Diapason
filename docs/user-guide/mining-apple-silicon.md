...
README. Then write Diapason' mining config:
...
```bash
diapason mine doctor        # capability matrix
diapason mine start         # launch gateway + miner-loop subprocesses
diapason mine status        # check sidecar + gateway metrics
diapason mine logs -n 120   # print recent logs
diapason mine stop          # stop mining subprocesses
```
