...
`diapason.learning.spec_search.composite_reward.score_batch`.
...
| `judge` | `MagicMock` | `diapason.evals.core.scorer.LLMJudgeScorer(...)` (or a deterministic scorer if your benchmark provides one) |
...
`src/diapason/evals/backends/diapason_agent.py` previously hardcoded
`builder.telemetry(telemetry).traces(True).build()`, ignoring the
`telemetry` parameter. This silently caused every agent-backend
evaluation to write to `~/.diapason/traces.db` regardless of caller
...
- `examples/diapason/spec_search_quickstart.py` — runnable end-to-end demo.
- `configs/diapason/examples/spec-search-quickstart.toml` — prebuilt config.
- `src/diapason/learning/spec_search/orchestrator.py` — `SpecSearchOrchestrator` (single session).
- `src/diapason/learning/spec_search/multi_session.py` — `SpecSearchLoop` (Algorithm 1).
- `src/diapason/learning/spec_search/composite_reward.py` — paper Eq. 1.
- `src/diapason/learning/spec_search/gate/benchmark_gate.py` — `GateOK` predicate.
- `src/diapason/learning/spec_search/external_adapter.py` — corpus → trace adapter.
