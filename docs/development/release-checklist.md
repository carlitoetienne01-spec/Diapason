...
2. **`diapason` (no args)** drops into a chat session within 2 s.
...
6. **`diapason doctor`** exits 0 once all bg work completes; shows the Background tasks table.
7. **Re-run `curl … | bash`** on the same machine. It completes ≤ 30 s, says `[ok] step already done` for every step.
8. **`diapason-uninstall`** removes `~/.diapason/` and `~/.local/bin/diapason*`. Verify with `ls`.
...
- Delete `~/.diapason/config.toml` — verify bare `diapason` re-runs init.
- Delete `~/.diapason/.venv` — verify re-running curl heals it.
