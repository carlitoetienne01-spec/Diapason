...
prove Diapason wiring; this validates that a real H100/H200 host can mine
through Pearl and serve inference through Diapason.
...
- Docker container `diapason-pearl-miner` starts.
- `~/.diapason/runtime/mining.json` is written.
...
## Verify Diapason Inference Uses Mining Endpoint
...
- If metric names differ, attach `/tmp/pearl-gateway-metrics.txt` to the PR and
  update `src/diapason/mining/_metrics.py`.
...
- Container stops.
- Sidecar is removed.
- `diapason ask` no longer routes through `vllm-pearl-mining` unless another
...
- Diapason inference succeeds through `vllm-pearl-mining`.
