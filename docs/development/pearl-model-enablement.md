...
through Pearl's vLLM miner and Diapason.

Diapason can point `vllm-pearl` at a model id, but a raw Hugging Face model is
...
Diapason only supports Pearl models published by the `pearl-ai` Hugging Face
organization. Private staging artifacts and Diapason-specific conversion
...
`diapason mine start`, vLLM `/v1/models`, Diapason inference routing, Pearl
gateway template refresh, and `diapason mine validate-model`.

`pearl-ai/Gemma-4-31B-it-pearl` and
`pearl-ai/Llama-3.1-8B-Instruct-pearl` are listed because they are public Pearl
org artifacts. They remain `planned` in Diapason until we have clean
...
2. Convert the target model.
   ...
   Diapason includes an experimental local converter for this work:
   ...
   its output as a staging artifact until `diapason mine inspect-model` and
   `diapason mine validate-model` pass on H100/H200 hardware.
...
3. Validate the Pearl vLLM plugin path.
   - Run `diapason mine inspect-model --model <pearl-model-id>
...
4. Validate chain integration.
   ...
   - `diapason mine status` parses those metrics.

5. Promote the model in Diapason.
...
`diapason mine models` renders that registry. Planned models are visible to users
...
Attach the JSON artifact from `diapason mine validate-model --output` to the issue.
