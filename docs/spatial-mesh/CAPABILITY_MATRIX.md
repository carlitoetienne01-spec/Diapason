...
| **Fichiers de l'app** | ✅ | ✅ | ❌ | ❌ | ❌ | Le transfert écrit dans le dossier privé `.diapason/transfers` de chaque système, sous un nom assaini, jamais en écrasant. Sur Mac, le dialogue Tauri peut désormais préparer un fichier, une photo ou une vidéo que le geste envoie ; Windows reçoit aujourd'hui et peut toujours émettre par la CLI. Accord humain obligatoire, plafond 2 Gio, confirmation finale signée. **Aucune capacité** — et plus aucune ne prétend le contraire. |
