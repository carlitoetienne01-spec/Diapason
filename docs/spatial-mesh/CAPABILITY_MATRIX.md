...
| **Curseur / clic / double-clic / défilement** | ⚠️ | ❌ | ❌ | ❌ | ❌ | Livré et testé automatiquement dans un mode `POINTER` distinct : index seul, pincement confirmé, maintien pour défiler, injection Tauri par Core Graphics et refus explicite sans Accessibilité. ⚠️ jusqu'au banc physique après reconstruction ; écran principal seulement. Windows n'a pas encore de suivi de main. |
