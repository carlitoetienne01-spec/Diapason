...
| Moteurs de gestes (`gestes_main.py`, `pointeur_main.py`) | ✅ automatisé | Transfert : 16 tests. Pointeur : 24 tests, machine indépendante pour qu'un clic ne puisse pas déposer. |
...
| Curseur, clic, ouverture et défilement | ⚠️ automatisé | Vocabulaire macOS (Auto ou verrou), injection Core Graphics. Le 29 août : Auto branché ; pincement sans plafond de durée. |
...
**Le mur est tombé le 25 août 2026 — option A retenue, détaillée dans
[`GESTES.md`](GESTES.md).** L'application capture, le serveur voit : la clé
et l'entitlement vivent dans le paquet Tauri, que macOS reconnaît comme une
application et à qui il accepte donc de poser la question. Constaté le
30 août sur le bundle installé :

```bash
plutil -extract NSCameraUsageDescription raw /Applications/Diapason.app/Contents/Info.plist
```

**Ce paragraphe a décrit un mur inexistant pendant cinq jours**, dans le
document qui sert de point d'entrée au chantier — pendant que quatre commits
de gestes le franchissaient. C'est le mode de panne que ce dossier connaît
déjà : un document qui dit ce qui était vrai le jour où on l'a écrit continue
d'instruire ceux qui le lisent ensuite. Une session qui l'aurait cru serait
partie chercher une sortie déjà prise.
