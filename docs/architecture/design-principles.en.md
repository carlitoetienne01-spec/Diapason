...
Diapason follows a set of design principles that guide every architectural decision. These principles ensure the framework remains extensible, portable, and easy to work with.

---

## 1. Pluggable Everything

Every major component in Diapason is defined as an **abstract base class** (ABC) with concrete implementations registered at runtime. This means you can swap, extend, or replace any part of the system without modifying existing code.
...
Diapason is designed to work **entirely without network access**. All core functionality -- inference, memory, agents, tools, telemetry -- operates locally. Cloud APIs are optional extensions, never requirements.
...
j = Diapason(engine_key="ollama")  # Local Ollama server
...
Diapason **auto-detects system hardware** at startup and recommends the optimal inference engine. The `detect_hardware()` function probes:
...
This recommendation is written to `config.toml` during `diapason init` and used as the default engine:

```bash
diapason init --force
# Detects hardware, writes ~/.diapason/config.toml with:
# [engine]
...
```bash
diapason telemetry stats          # Aggregated statistics
diapason telemetry export --json  # Export all records
```
...
Diapason provides a **clean Python API** through the `Diapason` class. There is no framework lock-in -- the SDK is a standard Python package with dataclass-based types and no required web framework.
...
- **Optional dependencies** via extras (`diapason[server]`, `diapason[memory-colbert]`, etc.)
...
The API server (`diapason serve`) implements the **OpenAI chat completions API format**, making Diapason a drop-in replacement for OpenAI in existing applications.
...
Any OpenAI client library can connect to Diapason:
...
Diapason requires **no external services** for core functionality. Everything needed to run the system is included or uses standard system libraries.
...
The only external requirement is a running inference engine (Ollama, vLLM, etc.), which is the model server itself -- not a dependency of Diapason.

Optional features that require additional packages:

| Feature | Extra | Packages |
|---------|-------|----------|
| FAISS memory | `diapason[memory-faiss]` | `faiss-cpu`, `sentence-transformers` |
| ColBERT memory | `diapason[memory-colbert]` | `colbert-ai`, `torch` |
| BM25 memory | `diapason[memory-bm25]` | `rank-bm25` |
| API server | `diapason[server]` | `fastapi`, `uvicorn` |
| Cloud inference | `diapason[inference-cloud]` | `openai`, `anthropic`, `google-genai` |
| vLLM engine | `diapason[inference-vllm]` | `vllm` |
| PDF ingestion | `diapason[memory-pdf]` | `pdfplumber` |
