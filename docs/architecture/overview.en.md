...
description: The five-primitive architecture behind Diapason — Intelligence, Engine, Agents, Tools, and Learning
search:
  boost: 2
---
...
Diapason is a research framework for studying on-device AI systems. Its architecture is organized around **five core abstractions** -- Intelligence, Engine, Agentic Logic, Memory, and Learning -- that work together through trace-driven feedback.

![Diapason Architecture](../assets/Diapason_Architecture.png)
...
The sandbox module (`diapason.sandbox`) adds a `SandboxedAgent` wrapper that runs any `BaseAgent` inside a Docker or Podman container with mount-security enforcement, and a `ContainerRunner` that manages the container lifecycle.
...
All extensible components in Diapason use a **decorator-based registry** for runtime discovery. The pattern is implemented in `RegistryBase[T]`, a generic base class that provides isolated storage per typed subclass.
...
src/diapason/
    core/               Core infrastructure shared by all primitives
        ...
        config.py           DiapasonConfig, hardware detection, TOML loading
    ...
    cli/                CLI commands (Click-based)
        ask.py              diapason ask -- query the assistant
        serve.py            diapason serve -- start API server
