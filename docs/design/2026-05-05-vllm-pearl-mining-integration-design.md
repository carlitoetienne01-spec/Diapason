...
### In scope (v1)
- New `diapason.mining` subsystem with `MiningProvider` ABC, `MinerRegistry`, `MiningCapabilities` / `MiningConfig` / `MiningStats` dataclasses
- `vllm-pearl` provider implementation: orchestrate Pearl's published `vllm-miner` Docker container
- `[mining]` TOML section in OJ config; `MiningConfig` field in `DiapasonConfig`
- New CLI namespace: `diapason mine init|start|stop|status|doctor|attach|logs`
- Runtime sidecar at `~/.diapason/runtime/mining.json` for engine ↔ mining handoff
...
| Engine attachment | Runtime sidecar JSON at `~/.diapason/runtime/mining.json` | Existing vLLM engine class stays untouched. Sidecar is the single source of truth tying mining lifecycle to engine resolution. Inspectable via `cat`. |
...
`MinerRegistry` added to `src/diapason/core/registry.py` as a peer to `EngineRegistry`, `AgentRegistry`, etc. `tests/conftest.py`'s autouse `_clean_registries` fixture is updated to include `MinerRegistry.clear()`.
...
`~/.diapason/runtime/mining.json` (created on `mine start`, removed on `mine stop`):
...
4. `diapason ask` and the SDK route to that endpoint transparently. The user's normal inference is the mining work.
...
Power users running their own Pearl container skip `diapason mine start` and write the sidecar themselves via `diapason mine attach --vllm-endpoint=... --gateway-url=...`. Decouples lifecycle from wiring.
...
| Command | Purpose |
|---|---|
| `diapason mine init` | Interactive: hardware/Docker checks, prompt for wallet + pearld credentials, write `[mining]`, pull/build image. Does NOT start mining. Pre-checks `>=200 GB` free disk. |
| `diapason mine start` | Launch container via the registered provider, write sidecar, print endpoint info. Idempotent if running. |
| `diapason mine stop` | Stop container, remove sidecar. Idempotent if not running. |
| `diapason mine status` | Read sidecar + query gateway `:8339/metrics`. Print `MiningStats`. |
| `diapason mine doctor` | Capability matrix; every check ✓/✗ with reason. Works in any state. |
| `diapason mine attach` | Manual mode: write sidecar without launching. |
| `diapason mine logs [-f]` | Tail container logs through Docker SDK. |
...
Image
  diapason/pearl-miner:<ref>   present (built 2026-04-30)   ✓
Pearl node
...
- Existing `com.diapason.gateway` daemon is unchanged.
...
- launchd/systemd installation surface (`diapason daemon install`) untouched.

### 6.5 Concurrency

POSIX `flock` on `~/.diapason/runtime/mining.lock` prevents racing `mine start` invocations.

### 6.6 `diapason ask` UX hint

When `[mining]` is configured but no sidecar exists, `cli/hints.py` emits one line: `"mining configured but not running — start it with \`diapason mine start\`"`. One-line UX nudge, no new infrastructure.
...
| **Build-from-pin** | OJ git-clones Pearl at a pinned ref into `~/.diapason/cache/pearl/`, then `docker buildx build` | v1 default (Pearl publishes nothing today) |
...
- **`auto_remove=False`** so a crashed container stays around for `diapason mine logs` post-mortem.
...
- `diapason mine init` triggers `ensure_image()`, streams build/pull output through the CLI with a clear time estimate (`"Building Pearl miner image — first run takes ~45 min on a fast machine"`).
- `diapason mine doctor` reports `image: present (tag, age, sha)` or `image: missing (run mine init)`.
- `diapason mine prune` (v1.x) cleans old `diapason/pearl-miner:*` tags. Manual `docker image rm` works in v1.
...
- **v1: on-demand only.** `diapason mine status` makes one HTTP GET per call (~10 ms). No background polling.
...
- `diapason telemetry stats --mining` (v1.x) joins to the latest `MiningStats` snapshot and reports `tokens / share`, `joules / share`, `est. PRL / kWh`.
...
**1. `submit_target` parsed into a tagged union; only one variant works.** `SoloTarget` accepted at runtime in v1; `PoolTarget` raises `NotImplementedError("pool support is v2 — track diapason#XYZ")`. Reachable only by users who edit their config to opt in.
...
| Concurrent `mine start` | POSIX `flock` on `~/.diapason/runtime/mining.lock`; second invocation errors clearly | clear message |
