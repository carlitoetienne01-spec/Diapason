...
[`src/diapason/analytics/events.py`](../src/diapason/analytics/events.py).
...
1. [`src/diapason/analytics/redaction.py`](../src/diapason/analytics/redaction.py) — value-level pattern matching (20+ regexes for PII).
2. [`src/diapason/analytics/events.py`](../src/diapason/analytics/events.py) — structural allowlist (event name + property name + type validator).
...
- Local telemetry (FLOPs, energy, latency stored in
  ...
  in [`src/diapason/telemetry/`](../src/diapason/telemetry/). It
