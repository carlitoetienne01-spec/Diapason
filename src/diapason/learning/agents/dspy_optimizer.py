...
from diapason.learning._stubs import AgentLearningPolicy
...
class DSPyAgentOptimizer:
    ...
    def optimize(self, trace_store: Any) -> Dict[str, Any]:
        ...
        if not HAS_DSPY:
            return {
                "status": "error",
                "reason": (
                    "dspy not installed (pip install 'diapason[learning-dspy]')"
                ),
            }
    ...
    def _write_configs(self, config_updates: Dict[str, Any]) -> None:
        ...
        from diapason.learning.agents.agent_evolver import (
            AgentConfigEvolver,
        )
