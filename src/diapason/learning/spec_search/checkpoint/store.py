...
A thin wrapper over a local git repository at ``<diapason_home>/.git``.
...
from diapason.learning.spec_search.storage.paths import (
    ConfigurationError,
    _find_source_root,
)
...
class CheckpointStore:
    ...
    def init(self) -> None:
        ...
        Refuses to initialize if ``self.root`` is inside the Diapason source
        ...
        if source_root is not None:
            try:
                ...
                raise ConfigurationError(
                    ...
                    f"Diapason source tree ({source_root}). Refusing to "
        ...
        if not (self._root / ".git").exists():
            self._git("init", "-q")
            self._git("config", "user.email", "spec-search@diapason.local")
            self._git("config", "user.name", "Diapason Spec Search")
        ...
        # Allow empty so init succeeds even on a brand-new diapason home.
        self._git(
            "commit",
            "--allow-empty",
            "-q",
