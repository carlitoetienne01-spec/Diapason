...
logger = logging.getLogger("diapason.server.agent_manager")
...
def _make_lightweight_system(
    ...
    try:
        ...
        try:
            from diapason.core.events import get_event_bus
...
def _ensure_registries_populated() -> None:
    ...
    try:
        import diapason.channels  # noqa: F401
    except Exception:
        pass
    ...
    # Also try to import browser tools (not included in diapason.tools.__init__)
    for _browser_mod in ("diapason.tools.browser", "diapason.tools.browser_axtree"):
        try:
    ...
    if not ChannelRegistry.keys():
        for mod_name in list(sys.modules):
            if mod_name.startswith("diapason.channels.") and not mod_name.endswith(
                "_stubs"
            ):
                try:
    ...
    if not any(ToolRegistry.contains(n) for n in _BROWSER_SUB_TOOLS):
        for _browser_mod in (
            "diapason.tools.browser",
            "diapason.tools.browser_axtree",
        ):
...
def build_tools_list() -> List[Dict[str, Any]]:
    ...
    from diapason.core.credentials import TOOL_CREDENTIALS
    from diapason.core.registry import ChannelRegistry, ToolRegistry
...
def _get_mcp_tools(app_state: Any) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    ...
    from diapason.mcp.transport import StdioTransport, StreamableHTTPTransport
    from diapason.tools.mcp_adapter import MCPToolProvider
    ...
    async def generate():
        ...
        while turns < max_turns:
            ...
            if tool_call_fragments and current_finish_reason == "tool_calls":
                ...
                from diapason.core.types import ToolCall as MsgToolCall
...
def create_agent_manager_router(
    ...
    async def bind_channel(
        ...
        if req.channel_type == "imessage":
            ...
            if identifier:
                try:
                    from diapason.channels.imessage_daemon import (
                        is_running,
                        run_daemon,
                    )
        ...
        if req.channel_type == "sendblue":
            ...
            if api_key_id and api_secret_key:
                try:
                    from diapason.channels.sendblue import (
                        SendBlueChannel,
                    )
        ...
        if req.channel_type == "slack":
            ...
            if bot_token and app_token:
                try:
                    from diapason.channels.slack_daemon import (
                        start_slack_daemon,
                    )
                    from diapason.channels.slack_daemon import (
    ...
    async def save_tool_credentials(tool_name: str, request: Request):
        from diapason.core.credentials import save_credential
    ...
    def remove_tool_credential(tool_name: str, key: str):
        from diapason.core.credentials import delete_credential
    ...
    def credential_status(tool_name: str):
        from diapason.core.credentials import get_credential_status
