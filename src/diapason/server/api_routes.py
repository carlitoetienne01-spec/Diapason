...
async def list_agents(request: Request):
    ...
    registered = []
    try:
        import diapason.agents  # noqa: F401 — side-effect registration
    ...
    running = []
    try:
        from diapason.tools.agent_tools import _SPAWNED_AGENTS
...
async def create_agent(req: AgentCreateRequest, request: Request):
    """Spawn a new agent."""
    try:
        from diapason.tools.agent_tools import AgentSpawnTool
...
def _get_memory_backend(request: Request):
    ...
    if backend is None:
        from diapason.tools.storage._stubs import MemoryBackendUnavailable
...
async def install_skill(request: Request):
    ...
    return {
        ...
        "message": "Use TOML files in ~/.diapason/skills/",
    }
...
async def list_sessions(request: Request, limit: int = 20):
    ...
    try:
        from diapason.sessions.store import SessionStore
...
def include_all_routes(app) -> None:
    ...
    try:
        ...
        logger.debug("voice live routes not available", exc_info=True)
    ...
    try:
        if hasattr(app.state, "agent_manager") and app.state.agent_manager:
            from diapason.server.agent_manager_routes import (  # noqa: PLC0415
