...
class SystemBuilder:
    ...
    def _resolve_memory(self, config):
        try:
            import diapason.tools.storage  # noqa: F401 -- trigger registration
    ...
    def _resolve_channel(self, config, bus):
        ...
        try:
            import diapason.channels  # noqa: F401 -- trigger registration
            from diapason.core.registry import ChannelRegistry
            from diapason.system._channel_kwargs import build_channel_kwargs
    ...
    def _setup_sandbox(self, config):
        ...
        if not sandbox_enabled:
            return None
        try:
            from diapason.sandbox.runner import ContainerRunner
    ...
    def _setup_learning_orchestrator(config: JarvisConfig):
        ...
        try:
            from diapason.core.config import DEFAULT_CONFIG_DIR
            from diapason.learning.learning_orchestrator import (
                LearningOrchestrator,
            )
            from diapason.learning.training.lora import LoRATrainingConfig
            from diapason.traces.store import TraceStore
