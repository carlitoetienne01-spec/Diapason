...
for _mod in ("cartesia_tts", "kokoro_tts", "openai_tts", "elevenlabs_tts"):
    try:
        importlib.import_module(f".{_mod}", __name__)
    except ImportError:
        pass

# Realtime duplex voice (Gemini Live / OpenAI Realtime) — import is light.
try:
    importlib.import_module(".realtime", __name__)
except ImportError:
    pass
