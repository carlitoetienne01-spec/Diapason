...
if TYPE_CHECKING:
    # Only used by type-checkers (mypy/pyright) for the ``DiapasonConfig.mining``
...
class ACEOptimizerConfig:
    ...
    # most Diapason users have credentials for.
    ...
    # Empty string defaults to ``~/.diapason/learning/ace/<task>/``.
    save_dir: str = ""
...
class StorageConfig:
    ...
    # the background and persists them across sessions (see diapason.memory).
...
class AgentConfig:
    ...
    default_system_prompt: str = (
        ...
        "who or what you are, identify yourself as Diapason. Respond "
...
class WhatsAppBaileysChannelConfig:
    ...
    auth_dir: str = ""  # Defaults to ~/.diapason/whatsapp_auth
...
class SchedulerConfig:
    ...
    db_path: str = ""  # Defaults to ~/.diapason/scheduler.db
...
class HeartbeatConfig:
    ...
    workspace_dir: str = ""  # empty → ~/.diapason/workspace
    speak: bool = False
    notify: bool = True
...
class RoutinesConfig:
    ...
    enabled: bool = True
    path: str = ""  # empty → <workspace>/ROUTINES.json
    state_path: str = ""
...
class WakeWordConfig:
    ...
    action: str = "talk"  # talk | dictation
    cooldown_s: float = 2.5
    sensitivity: float = 0.5
    device: str = ""
    model_path: str = ""
    require_local_only: bool = True
    # PTT dictation: strip wake prefix and prefer command path
    text_gate: bool = True
...
class DesktopVisionConfig:
    ...
    enabled: bool = False
    monitor: int = 1  # 1-based; 0 = default display
    max_dimension: int = 1280
    allow_cloud: bool = False
    engine: str = ""  # empty → [engine].default
    model: str = ""  # empty → [intelligence].default_model
    redact: bool = False  # reserved
    keep_temp: bool = False
    rate_limit_ms: int = 1500
    # Continuous share (start/stop by voice)
...
class DictationConfig:
    ...
    dictionary_path: str = ""  # empty → ~/.diapason/dictation_dictionary.json
    llm_polish: bool = False  # opt-in; Diapason default is also off
    email_mode: str = "off"  # "off" | "on" | "auto"
    llm_timeout_ms: int = 2000
    paste_to_frontmost: bool = True
    hotkey: str = "Cmd+Alt+Space"
    ptt_hold: bool = True
    voice_commands: bool = True
    # Learn STT→polished / user corrections into the dictionary
...
# Sections that users may set via ``diapason config set``.
# ``hardware`` is auto-detected and not user-settable.
_SETTABLE_SECTIONS = frozenset(DiapasonConfig.__dataclass_fields__.keys()) - {
    "hardware",
    "mining",
}
...
def validate_config_key(dotted_key: str) -> type:
    ...
    for i, part in enumerate(parts):
        ...
        if isinstance(fld_type, str):
            ...
            import diapason.core.config as _cfg_mod
...
def _migrate_toml_data(data: Dict[str, Any], cfg: "DiapasonConfig") -> None:
...
def load_config(path: Optional[Path] = None) -> DiapasonConfig:
    ...
    _ensure_config_dir()
    hw = detect_hardware()
    cfg = DiapasonConfig(hardware=hw)
...
# ---------------------------------------------------------------------------
# Default TOML generation (for ``diapason init``)
# ---------------------------------------------------------------------------
...
# background and persists them. Starts/stops with `diapason serve` and
# `diapason chat`; manage stored facts with `diapason memory list` / `clear`.
[memory]
...
# enabled = false
# poll_interval = 60
# db_path = ""                # Defaults to ~/.diapason/scheduler.db

# [channel.whatsapp_baileys]
# auth_dir = ""               # Defaults to ~/.diapason/whatsapp_auth
