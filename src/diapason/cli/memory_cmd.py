"""``diapason memory`` — memory management subcommands."""
...
def _get_fact_store():
    ...
    from diapason.memory.store import create_fact_store

    config = load_config()
    mem = config.memory
    return create_fact_store(
        getattr(mem, "backend", "local"),
        path=getattr(mem, "facts_path", "~/.diapason/memory_facts.jsonl"),
