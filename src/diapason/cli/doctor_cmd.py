...
from diapason.core.config import DEFAULT_CONFIG_PATH, load_config
...
def _ensure_engines_imported() -> None:
    ...
    try:
        import diapason.engine  # noqa: F401
    except Exception:
        pass
...
def _check_engines() -> List[CheckResult]:
    ...
    from diapason.engine import _discovery
...
def _check_optional_deps() -> List[CheckResult]:
    ...
    results: List[CheckResult] = []
    optional_packages = [
        ("fastapi", "diapason[server]", "REST API server"),
        ("torch", "pip install torch", "SFT/GRPO training"),
        ("pynvml", "diapason[gpu-metrics]", "NVIDIA energy monitoring"),
        ("amdsmi", "diapason[energy-amd]", "AMD energy monitoring"),
        ("colbert", "diapason[memory-colbert]", "ColBERT memory backend"),
        ("zeus", "diapason[energy-apple]", "Apple Silicon energy monitoring"),
    ]
...
def _check_desktop_vision() -> CheckResult:
    try:
        from diapason.core.config import load_config

        v = load_config().desktop.vision
        if not v.enabled:
            return CheckResult(
                "Desktop vision",
                "ok",
        ...
        return CheckResult(
            "Desktop vision",
            "ok",
            f"Enabled · {model} · {cloud}",
            details="Needs Screen Recording permission for screencapture",
        )
    except Exception as exc:
        return CheckResult("Desktop vision", "warn", str(exc))


def _check_dictation() -> CheckResult:
    try:
        from diapason.core.config import load_config

        d = load_config().dictation
        bits = [
            "polish" if d.polish else "no-polish",
            "dict" if d.dictionary else "no-dict",
            f"email={d.email_mode}",
        ]
        if d.llm_polish:
            bits.append("llm")
        return CheckResult("Dictation", "ok", " · ".join(bits))
    except Exception as exc:
        return CheckResult("Dictation", "warn", str(exc))


def _check_heartbeat_routines() -> CheckResult:
    try:
        from diapason.core.config import load_config

        cfg = load_config()
        hb = "on" if cfg.heartbeat.enabled else "off"
        rt = "on" if cfg.routines.enabled else "off"
        return CheckResult(
            "Heartbeat / routines",
            "ok",
...
def _check_wake_backend() -> CheckResult:
    try:
        from diapason.core.config import load_config
        from diapason.speech.wakeword import openwakeword_available

        backend = (load_config().speech.wakeword.backend or "phrase_gate").strip()
        oww = openwakeword_available()
        if backend in ("openwakeword", "oww", "ml") and not oww:
            return CheckResult(
                "Wake backend",
                "warn",
                f"{backend} configured but openWakeWord missing",
                details="uv sync --extra speech-wake",
            )
        return CheckResult(
            "Wake backend",
            "ok",
            f"{backend} · openWakeWord={'yes' if oww else 'no'}",
        )
    except Exception as exc:
        return CheckResult("Wake backend", "warn", str(exc))
...
def _run_all_checks() -> List[CheckResult]:
    ...
    checks.append(_check_desktop_vision())
    checks.append(_check_dictation())
    checks.append(_check_heartbeat_routines())
    checks.append(_check_wake_backend())
...
def doctor(as_json: bool) -> None:
    ...
    from diapason.cli._bg_state import get_status
