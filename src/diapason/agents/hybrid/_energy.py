...
* Samples **all visible NVIDIA GPUs on the host where the runner runs**.
  ...
  ``DIAPASON_HYBRID_ENERGY=0`` to silence the warning.
...
class EnergyCollector:
    ...
    def __init__(self, sample_hz: float = 2.0) -> None:
        ...
        self._enabled = (_env_get("HYBRID_ENERGY", "1") or "1") != "0"
        self._pynvml = None
        self._handles: list = []
    ...
    def __enter__(self) -> "EnergyCollector":
        ...
        try:
            ...
            _log_once(
                ...
                "energy_j_total will be 0. Set DIAPASON_HYBRID_ENERGY=0 to silence."
            )
            self._pynvml = None
            return self
