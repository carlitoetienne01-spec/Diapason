...
both forced by the Diapason environment and documented inline:
...
* in-tool correctness check — the original ``answer`` tool LLM-judges the
  prediction against the gold answer inside ``call_tool``. Diapason
...
def run_answer(
    ...
    *,
    ...
    in-tool LLM correctness check is dropped — Diapason scores
    downstream.
    """
...
def run_search(
    ...
    *,
    ...
    Diapason substitution for the missing FAISS wiki index).
    """
    prompt = (
        context_str.strip()
        + "\n\n"
