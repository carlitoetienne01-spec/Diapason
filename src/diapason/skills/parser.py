...
2. Tolerant pass — maps non-spec top-level fields to their canonical
   locations under metadata.diapason.* via FIELD_MAPPING.
...
class SkillParser:
    ...
    def _build_manifest(
        ...
        captures unknown fields under metadata.diapason.original_frontmatter,
        and merges metadata.diapason.* into canonical fields.
        """
        ...
        if not isinstance(raw_metadata, dict):
            raw_metadata = {}
        # Initialize diapason namespace
        oj_meta = dict(raw_metadata.get("diapason") or {})
        ...
        for key, value in frontmatter.items():
            ...
            if key in FIELD_MAPPING:
                ...
                LOGGER.warning(
                    ...
                    "(value preserved in metadata.diapason.original_frontmatter)",
                    key,
                    manifest.name,
                )
        ...
        # Preserve unmapped fields under metadata.diapason.original_frontmatter
        if unmapped:
            oj_meta["original_frontmatter"] = unmapped

        # Stash the diapason metadata block back
        if oj_meta:
            new_metadata = dict(raw_metadata)
            new_metadata["diapason"] = oj_meta
