...
Uses OAuth 2.0 tokens stored locally (see :mod:`diapason.connectors.oauth`).
...
from diapason.connectors.google_auth import (
    GoogleAuthError,
)
from diapason.connectors.google_auth import (
...
class GmailConnector(BaseConnector):
    ...
    credentials_path:
        ...
        ``~/.diapason/connectors/gmail.json``.
    """
    ...
    def is_connected(self) -> bool:
        ...
        token), which made `diapason connect gmail` short-circuit with
    ...
    def _refresh_token(self) -> str:
        ...
        if not new:
            raise RuntimeError(
                "Gmail token refresh failed — re-run `diapason connect gmail`"
            )
        return new
