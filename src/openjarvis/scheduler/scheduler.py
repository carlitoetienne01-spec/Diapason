    ...
    def _execute_task(self, task: ScheduledTask) -> None:
        ...
        try:
            ...
            if kind == "heartbeat":
                ...
                tick = run_heartbeat_tick(system=self._system)
                result_text = tick.get("content") or tick.get("reason") or str(tick)
                success = bool(tick.get("ok", True))
            elif kind == "routine":
                from openjarvis.heartbeat.kinds import run_routine
                from openjarvis.heartbeat.routines import get_routine

                rid = str(meta.get("routine_id") or "").strip()
                routine = get_routine(rid) if rid else None
                if routine is None:
                    result_text = f"Unknown routine: {rid}"
                    success = False
                else:
                    out = run_routine(routine, system=self._system)
                    result_text = out.get("content") or out.get("reason") or str(out)
