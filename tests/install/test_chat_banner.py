...
def test_banner_empty_when_all_ready(tmp_diapason_home: Path) -> None:
...
def test_banner_shows_rust_building(tmp_diapason_home: Path) -> None:
...
def test_banner_shows_model_downloading(tmp_diapason_home: Path) -> None:
    (tmp_diapason_home / ".state" / "extension-built").write_text("")
...
def test_banner_shows_failed_in_dim_warning(tmp_diapason_home: Path) -> None:
    (tmp_diapason_home / ".state" / "extension-failed").write_text("error tail")
    s = _bg_state.get_status()
    banner = render_startup_banner(s)
