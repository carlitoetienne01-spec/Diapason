...
def test_get_status_empty(tmp_diapason_home: Path) -> None:
...
def test_get_status_rust_built(tmp_diapason_home: Path) -> None:
    (tmp_diapason_home / ".state" / "extension-built").write_text("")
    s = _bg_state.get_status()
    assert s.rust_extension == "ready"


def test_get_status_rust_failed(tmp_diapason_home: Path) -> None:
    (tmp_diapason_home / ".state" / "extension-failed").write_text("error tail here")
    s = _bg_state.get_status()
...
def test_get_status_model_ready(tmp_diapason_home: Path) -> None:
    (tmp_diapason_home / ".state" / "models" / "qwen3.5:9b.ready").write_text("")
    s = _bg_state.get_status()
    assert s.models == {"qwen3.5:9b": "ready"}


def test_get_status_model_failed(tmp_diapason_home: Path) -> None:
    (tmp_diapason_home / ".state" / "models" / "qwen3.5:9b.failed").write_text(
        "net error"
    )
    s = _bg_state.get_status()
...
def test_get_status_ready_supersedes_downloading(tmp_diapason_home: Path) -> None:
    """If both .downloading and .ready exist (race window), .ready wins."""
    models_dir = tmp_diapason_home / ".state" / "models"
...
def test_all_ready_true_when_all_ready(tmp_diapason_home: Path) -> None:
    (tmp_diapason_home / ".state" / "extension-built").write_text("")
    (tmp_diapason_home / ".state" / "models" / "qwen3.5:9b.ready").write_text("")
    s = _bg_state.get_status()
...
def test_get_status_handles_deleted_file_race(tmp_diapason_home: Path) -> None:
    """File present at glob-time, gone at read-time = treat as missing."""
    f = tmp_diapason_home / ".state" / "extension-failed"
    f.write_text("oops")
