...
def test_writes_minimal_local_config(tmp_diapason_home: Path) -> None:
    hw = HardwareInfo(
        platform="linux",
        cpu_brand="AMD EPYC",
        cpu_count=16,
    ...
    cfg_path = tmp_diapason_home / "config.toml"
...
def test_writes_cloud_config(tmp_diapason_home: Path) -> None:
    ...
    data = tomllib.loads((tmp_diapason_home / "config.toml").read_text())
    ...
    assert "sk-ant-test" not in (tmp_diapason_home / "config.toml").read_text()


def test_includes_install_provenance(tmp_diapason_home: Path, monkeypatch) -> None:
...
def test_writes_seed_files_if_absent(tmp_diapason_home: Path) -> None:
    hw = HardwareInfo(platform="linux", cpu_brand="x", cpu_count=1, ram_gb=4.0)
    _bootstrap.write_initial_config(hardware=hw, engine="ollama", model="qwen3.5:2b")
    assert (tmp_diapason_home / "SOUL.md").exists()
    assert (tmp_diapason_home / "MEMORY.md").exists()
    assert (tmp_diapason_home / "USER.md").exists()
    assert (tmp_diapason_home / "skills").is_dir()


def test_does_not_overwrite_existing_seeds(tmp_diapason_home: Path) -> None:
    soul = tmp_diapason_home / "SOUL.md"
...
def test_overwrites_existing_config_toml(tmp_diapason_home: Path) -> None:
    cfg = tmp_diapason_home / "config.toml"
...
def test_handles_special_chars_in_model_name(tmp_diapason_home: Path) -> None:
