...
def tmp_openjarvis_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    monkeypatch.setattr("diapason.core.config.DEFAULT_CONFIG_DIR", home)
    monkeypatch.setattr("diapason.core.config.DEFAULT_CONFIG_PATH", config_path)
    # Also patch init_cmd's module-level bindings (imported with ``from ... import``).
    monkeypatch.setattr("diapason.cli.init_cmd.DEFAULT_CONFIG_DIR", home)
    monkeypatch.setattr("diapason.cli.init_cmd.DEFAULT_CONFIG_PATH", config_path)
    # Patch doctor_cmd's module-level bindings.
    monkeypatch.setattr("diapason.cli.doctor_cmd.DEFAULT_CONFIG_PATH", config_path)
