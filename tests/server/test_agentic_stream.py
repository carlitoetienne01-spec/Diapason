...
class TestLaCarteDitCeQueLaRechercheARendu:
    """S2 (22/09/2026) : `web_search` sait depuis le 20/09 quel moteur a
    répondu et combien de résultats il a rendus. Rien ne le faisait traverser
    jusqu'à la carte, qui affichait « web_search · 0,8 s » et rien d'autre :
    une recherche VIDE se lisait comme une recherche fructueuse, et la
    réponse bâtie dessus ne s'annonçait pas (§5)."""

    def test_le_moteur_et_le_compte_traversent(self):
        from diapason.core.types import ToolResult
        from diapason.server.agentic_stream import details_du_fil
    ...
    def test_zero_traverse_aussi(self):
        """C'est même le cas pour lequel ceci existe : un `if not nombre`
        l'aurait jeté."""
        from diapason.core.types import ToolResult
        from diapason.server.agentic_stream import details_du_fil
    ...
    def test_un_outil_qui_leve_n_a_pas_de_resultat(self):
        """`resultat` est nommé avant le try : sans cela, la carte d'un outil
        cassé serait un NameError au lieu d'un échec."""
        from diapason.server.agentic_stream import details_du_fil
