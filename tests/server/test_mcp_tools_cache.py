...
def test_returns_tools_from_mcp_server(mock_load_config: MagicMock):
    ...
    with (
        patch("diapason.mcp.transport.StreamableHTTPTransport"),
        patch("diapason.mcp.client.MCPClient"),
        patch("diapason.tools.mcp_adapter.MCPToolProvider") as MockProvider,
    ):
