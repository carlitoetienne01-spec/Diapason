...
def test_ensure_image_already_local():
    ...
    fake.images.get.return_value = MagicMock(
        id="sha256:abc", tags=["diapason/pearl-miner:main"]
    )
...
def test_ensure_image_pulls_if_published():
    from diapason.mining._docker import (
        APIError,
        ImageNotFound,
        NotFound,
        PearlDockerLauncher,
    ...
    launcher = PearlDockerLauncher(client=fake)
    with patch(
        "diapason.mining._docker._docker_error_types",
...
def test_docker_build_raises_nofile_limit(tmp_path):
    ...
    with patch("diapason.mining._docker.subprocess.run") as run:
        launcher._docker_build(tmp_path, "diapason/pearl-miner:test")
...
def test_launcher_start_mounts_local_model_path(_env_password, tmp_path):
    from diapason.mining._docker import LOCAL_MODEL_BIND_PATH, PearlDockerLauncher
    from diapason.mining._stubs import MiningConfig, SoloTarget
...
def test_launcher_stop_finds_named_container_without_in_memory_reference():
    ...
    launcher = PearlDockerLauncher(client=fake_client)
    launcher.stop()
    fake_client.containers.get.assert_called_once_with("diapason-pearl-miner")
    fake_container.stop.assert_called_once()
    fake_container.remove.assert_called_once()
    assert launcher._container is None
