"""Tests for the ``diapason eval`` CLI commands."""
...
class TestEvalCLI:
    ...
    def test_eval_group_exists(self):
        """``diapason eval --help`` shows run/compare/report/list subcommands."""
    ...
    def test_eval_list_benchmarks(self):
        """``diapason eval list`` exits 0 and outputs benchmark names."""
        ...
        assert "diapason-direct" in result.output
        assert "diapason-agent" in result.output

    def test_eval_run_missing_args(self):
        """``diapason eval run`` without required args fails gracefully."""
