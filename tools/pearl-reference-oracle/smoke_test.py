...
`pearl_mining` Python package. Used as Phase 0-B of the Diapason Apple Silicon
mining spec ([Spec B]).
...
def main() -> None:
    ...
    print(
        f"       proof.m={plain_proof.m} proof.n={plain_proof.n} "
        f"proof.k={plain_proof.k}  "
    ...
    if bt_deltas != COLS_PATTERN:
        _fail(
            f"bt.row_indices deltas ({bt_deltas}) != COLS_PATTERN ({COLS_PATTERN})",
            4,
        )
