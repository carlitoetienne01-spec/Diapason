/**
 * deep-link.ts — Handler for diapason:// deep links in the Tauri desktop app.
 *
 ...
 *   diapason://research/{session_id}   → open a research session
 *   diapason://connector/{connector_id} → open a connector settings panel
 */
...
/**
 * Parse an `diapason://` deep link URL into a structured target.
 *
 * @param url - The raw deep link URL string (e.g. `diapason://research/abc123`).
 * @returns A {@link DeepLinkTarget} if the URL is valid, or `null` if it
 *   cannot be parsed or does not use the `diapason:` scheme.
 *
 * @example
 * parseDeepLink("diapason://research/abc123");
 * // → { type: "research", id: "abc123" }
 *
 * parseDeepLink("diapason://connector/gmail");
...
export function parseDeepLink(url: string): DeepLinkTarget | null {
  try {
    ...
    if (parsed.protocol !== "diapason:") return null;

    // The URL constructor treats "diapason://research/abc123" such that
