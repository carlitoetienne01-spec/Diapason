...
#   DIAPASON_MOCK_LLM_URL - OpenAI-compatible endpoint (Ollama or vLLM)
#
# Optional:
#   DIAPASON_ALLOW_COMMIT_DRIFT=1  - bypass commit-pin enforcement

set -euo pipefail

: "${HERMES_AGENT_PATH:?must be set}"
: "${OPENCLAW_PATH:?must be set}"
: "${DIAPASON_MOCK_LLM_URL:?must be set (e.g. http://localhost:11434/v1)}"
...
uv run python -c "
from diapason.evals.comparison.third_party import (
...
for fwk in "${SMOKE_FRAMEWORKS[@]}"; do
  for bench in "${SMOKE_BENCHES[@]}"; do
    uv run python -m diapason.evals.comparison.make_configs \
      --framework "$fwk" \
      --model "$SMOKE_MODEL" \
      --benchmark "$bench" \
    ...
    DIAPASON_BACKEND_BASE_URL="$DIAPASON_MOCK_LLM_URL" uv run python - "$config" "$run_dir" <<'PY'
...
PY
    echo "  ▸ $fwk × $bench"
    mkdir -p "$run_dir"
    DIAPASON_BACKEND_BASE_URL="$DIAPASON_MOCK_LLM_URL" \
    DIAPASON_BACKEND_API_KEY="${DIAPASON_BACKEND_API_KEY:-smoke}" \
    uv run python -m diapason.evals run --config "$config" \
      || echo "    FAILED (continuing)"
  done
done

echo "==> Generating T1 from smoke results"
uv run python -m diapason.evals.comparison.table_gen \
