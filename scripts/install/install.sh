...
# uv/git tooling installs to Windows paths the rest of Diapason can't
...
Diapason runs on Windows via WSL2. Two paths:
...
Diapason installs to $HOME/.diapason, not /usr/local. Re-run as your
...
write_config() {
    "$VENV_DIR/bin/diapason" _bootstrap --write-config \
...
install_symlinks() {
    ...
    ln -sf "$SCRIPTS_DIR/diapason-wrapper.sh" "$HOME/.local/bin/diapason"
    ln -sf "$SCRIPTS_DIR/diapason-uninstall.sh" "$HOME/.local/bin/diapason-uninstall"
}

# Tracks whether the user needs to source ~/.bashrc / ~/.zshrc / open a
# new terminal before `diapason` will resolve. Set only when ensure_path
...
ensure_path() {
    ...
    if grep -q "Diapason" "$rc" 2>/dev/null; then
...
from diapason.core.config import detect_hardware, recommend_model
hw = detect_hardware()
...
step clone_repo         "Clone Diapason repo" clone_repo
step copy_scripts       "Copy install scripts"  copy_scripts
step create_venv        "Create venv"           create_venv
step editable_install   "Install Diapason"    editable_install
...
#
#   PATH ok + model ok   -> "type diapason"
#   PATH new + model ok  -> "source rc && diapason  (or open new terminal)"
#   PATH ok + model bad  -> "model still downloading; diapason doctor"
#   PATH new + model bad -> "source rc && diapason doctor; chat works once download finishes"
#
# `diapason` and `diapason doctor` need PATH equally, so the source/restart
# guidance goes first when PATH was modified.
NEXT_CMD="diapason"
if [[ "$MODEL_PULL_OK" -ne 1 ]]; then
    NEXT_CMD="diapason doctor"
fi
...
if [[ "$MODEL_PULL_OK" -ne 1 ]]; then
    cat <<EOF
NOTE: the qwen3.5:2b model didn't finish downloading. 'diapason doctor'
...
Background work continues silently:
  ...
  Run 'diapason doctor' to check status anytime.
EOF
