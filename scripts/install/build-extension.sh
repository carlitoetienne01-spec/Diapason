...
MANIFEST="$SRC_DIR/rust/crates/diapason-python/Cargo.toml"
...
if uv run maturin develop -m "$MANIFEST" >>"$LOG" 2>&1; then
    ...
    if ! uv run python -c "import diapason_rust" >>"$LOG" 2>&1; then
        rc=$?
        {
            echo "build-extension.sh: maturin succeeded but 'import diapason_rust'"
